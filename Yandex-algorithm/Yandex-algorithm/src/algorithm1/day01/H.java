...
// Поезда всегда ходят точно по расписанию, и Таня знает, что поезд стоит на платформе ровно одну минуту, а интервал между поездами (время, в течение которого поезда у платформы нет)
// составляет a минут для поездов на первом пути и b минут для поездов на втором пути.
...
//        func(1, 5, 1, 2); // -1
    ...
    public static void func(int aInterv, int bInterv, int aCnt, int bCnt) {

        int aMin = aCnt + aCnt * aInterv - aInterv;
        int aMax = aMin + aInterv * 2;

        int bMin = bCnt + bCnt * bInterv - bInterv;
        int bMax = bMin + bInterv * 2;

        int from = Integer.max(aMin, bMin);
        int to = Integer.min(aMax, bMax);

        if (from > to) { // (bMax < aMin || bMin > aMax)
